import {createSourceFile, project} from '../utils'
...
describe('migrateFile()', () => {
  ...
  test('Throws for already migrated file', async () => {
    ...
    await expect(migrateFile(project, sourceFile, true))
      .rejects
      .toThrow('File already migrated')
  })

  test('Throws for not supported files', async () => {
    await expect(migrateFile(project, createSourceFile('@Component', 'txt'), true))
      .rejects
  ...
  describe('Imports & Exports', () => {
    test('Empty exported class resolves', async () => {
      ...
      await expect( migrateFile(project, sourceFile, true))
        .rejects
        .toThrow('Non-default exported class not supported')
    })
